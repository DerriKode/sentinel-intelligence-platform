...
| D-012 | Stage 2 may not begin until `APPROVE STAGE 1` is observed | `STAGE_1_FOUNDATION_CHECKPOINT.md` | Satisfied 2026-07-17 |
...
| D-017 | Shared form/action primitives remain dependency-free and native-first; client validation and permission presentation are usability behavior only, while APIs retain all authorization and upload enforcement | `STAGE_2_FORM_AND_ACTION_COMPONENTS.md`, `apps/web/src/components/forms/` | Approved 2026-07-17 |
| D-018 | Shared data/feedback components use native semantics, explicit text states, bounded pagination, and focus-managed overlays; displayed counts, filters, visibility, and disabled states never replace API authorization or data-scope enforcement | `STAGE_2_DATA_AND_FEEDBACK_COMPONENTS.md`, `apps/web/src/components/data-feedback/` | Implemented |
| D-019 | The internal shell accepts display-ready context and capability-informed navigation, removes unavailable destinations from presentation, and treats all client visibility as a usability aid while APIs remain the authorization authority | `STAGE_2_INTERNAL_APPLICATION_SHELL.md`, `apps/web/src/components/shell/` | Approved 2026-07-17 |
| D-020 | The public shell remains separate under `/public/*`, exposes only approved public navigation and safe display-ready state, and enables a language only after reviewed translation is available | `STAGE_2_PUBLIC_PORTAL_SHELL.md`, `apps/web/src/components/public-shell/` | Approved 2026-07-17 |
| D-021 | Role dashboards are authorized, versioned read-model compositions with server-scoped widgets, source/freshness evidence, truthful partial states, safe drill-downs, and no new business truth, permission bypass, staff ranking, or autonomous decision output | `STAGE_2_DASHBOARD_DATA_ARCHITECTURE.md` | Locked for Stage 2 |
